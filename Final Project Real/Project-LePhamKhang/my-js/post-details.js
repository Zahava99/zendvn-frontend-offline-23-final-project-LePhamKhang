...
//
function DisplayCommentsItems (Comments, ParentId = null, isParent = true) {
  const level = isParent ? 1 : 2
  const btnReply = Emails
  ?/*html*/ `    
  <div class="reply">
    <button class="replyComments" data-level="${level}" data-reply-email="${Comments.Email}" data-parent-id="${ParentId}">
    <i class="fa-regular fa-share"></i> Reply</button>
  </div>`
  : ''
  const publishDate = dayjs(Comments.dateTime).fromNow()
  ...
  //HTMLChild
  let htmlChild = ''
  if (isParent && Comments.childItems.length > 0) {
    Comments.childItems.forEach((HTMLCHILD,index) => {
      htmlChild += DisplayCommentsChildItems(HTMLCHILD,ParentId,false)
    })
  }
  return /*html*/ `
  ...
  <div class="content">
    <h5 class="title">${Comments.Email}</h5>
...
function DisplayCommentsChildItems (items,ParentId,isParent = false) {
  const level = isParent ? 1 : 2
  const btnReply = Emails
  ? `    
  <div class="reply">
    <button class="replyComments" data-level="${level}" data-reply-email="${items.Email}" data-parent-id="${ParentId}">
    <i class="fa-regular fa-share"></i> Reply</button>
  </div>`
  : ''
  const className = isParent ? 'comment' : 'reply'
